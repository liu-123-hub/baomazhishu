...
class WebSocketClient {
  ...
  async _listenElectronPort() {
    if (!platform.isElectron || !window.electronAPI) return
    try {
      const portInfo = await window.electronAPI.getBackendPort()
      this._backendPort = portInfo.port || 8000
      if (typeof this._unsubElectronPort === 'function') {
        this._unsubElectronPort()
      }
      this._unsubElectronPort = window.electronAPI.onBackendPort?.((info) => {
        this._backendPort = info.port || 8000
      ...
      console.warn('[WS] Failed to get Electron backend port:', e)
    }
  }
  ...
  async init() {
    await this._listenElectronPort()
    this.connect()
  }
}
